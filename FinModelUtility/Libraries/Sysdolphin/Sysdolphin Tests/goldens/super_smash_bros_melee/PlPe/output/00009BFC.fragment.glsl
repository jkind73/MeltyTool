...
void main() {
  ...
  vec3 colorComponent = (vec3(0.7019608)*ambientLightColor.rgb + mergedLightDiffuseColor.rgb)*vec3(2.0)*texture(texture0, uv0).rgb + vec3(0.7019608)*texture(texture1, uv1).rgb*mergedLightSpecularColor.rgb;
