...
void main() {
  ...
  vec3 colorComponent = (vec3(0.5019608)*ambientLightColor.rgb + mergedLightDiffuseColor.rgb)*vec3(2.0)*texture(texture0.sampler, transformUv3d(texture0.transform3d, uv0)).rgb + vec3(1.0,0.9960784,0.0)*mergedLightSpecularColor.rgb;
