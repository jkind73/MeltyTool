...
void main() {
  ...
  vec3 colorComponent = (vec3(0.2)*ambientLightColor.rgb + mergedLightDiffuseColor.rgb)*vec3(2.0)*vec3(0.2980392)*texture(texture0, uv0).rgb + vec3(0.627451)*texture(texture0, uv0).rgb*mergedLightSpecularColor.rgb;
