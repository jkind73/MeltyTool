...
void main() {
  ...
  vec3 colorComponent = clamp(clamp((texture(texture0, uv0).rgb + texture(texture1, sphericalReflectionUv).rgb), 0.0, 1.0)*(ambientLightColor.rgb*vec3(0.4) + mergedLightDiffuseColor.rgb*vec3(0.4980392))*vec3(2.0), 0.0, 1.0);
