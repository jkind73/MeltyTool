...
void main() {
  ...
  vec3 colorComponent = clamp(clamp((ambientLightColor.rgb*vec3(0.4) + mergedLightDiffuseColor.rgb*vec3(0.4980392) + mergedLightSpecularColor.rgb)*texture(texture0, uv0).rgb*vec3(2.0), 0.0, 1.0) + texture(texture1, sphericalReflectionUv).rgb, 0.0, 1.0);
