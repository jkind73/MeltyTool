...
void main() {
  ...
  vec3 colorComponent = clamp(clamp(((ambientLightColor.rgb*vec3(0.4) + mergedLightDiffuseColor.rgb*vec3(0.4980392) + mergedLightSpecularColor.rgb)*texture(texture0, uv0).rgb*vec3(2.0)*vec3(scalar_3dsAlpha5) + color_3dsColor5), 0.0, 1.0)*vec3(scalar_3dsAlpha5) + color_3dsColor5, 0.0, 1.0);
