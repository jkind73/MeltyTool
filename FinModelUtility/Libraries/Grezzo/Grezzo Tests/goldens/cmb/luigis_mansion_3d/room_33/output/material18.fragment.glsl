...
void main() {
  ...
  vec3 colorComponent = clamp(clamp((color_3dsColor3 + vec3(texture(texture0, uv1).rgb.g)), 0.0, 1.0)*clamp(clamp((color_3dsColor2*vec3(1.0 + -1.0*vertexColor0.rgb.r*vertexColor0.rgb.r*2.0) + vertexColor0.rgb), 0.0, 1.0)*clamp((texture(texture2, uv0).rgb*(vec3(1.0) + vec3(-1.0)*clamp(vec3(texture(texture2, uv0).a)*vec3(vertexColor0.a), 0.0, 1.0)) + texture(texture1, uv0).rgb*clamp(vec3(texture(texture2, uv0).a)*vec3(vertexColor0.a), 0.0, 1.0)), 0.0, 1.0)*vec3(2.0)*(ambientLightColor.rgb + mergedLightDiffuseColor.rgb), 0.0, 1.0), 0.0, 1.0);
