...
uniform float scalar_3dsAlpha2;
...
void main() {
  ...
  vec3 colorComponent = clamp(color_3dsColor2*mergedLightSpecularColor.rgb*vec3(0.4980392) + clamp((ambientLightColor.rgb*vec3(0.8) + mergedLightDiffuseColor.rgb)*clamp(clamp((color_3dsColor3 + texture(texture0, uv0).rgb), 0.0, 1.0)*texture(texture1, uv0).rgb, 0.0, 1.0), 0.0, 1.0), 0.0, 1.0);

  float alphaComponent = texture(texture1, uv0).a*scalar_3dsAlpha1*scalar_3dsAlpha2;
