...
void main() {
  ...
  vec3 colorComponent = clamp(clamp((color_3dsColor3 + vec3(texture(texture0, uv0).rgb.g)), 0.0, 1.0)*clamp(clamp((vec3(texture(texture1, uv0).rgb.b)*texture(texture2, sphericalReflectionUv).rgb + clamp(color_3dsColor2*vec3(1.0 + -1.0*vertexColor0.rgb.r*vertexColor0.rgb.r*2.0) + clamp(vertexColor0.rgb, 0.0, 1.0), 0.0, 1.0)), 0.0, 1.0)*texture(texture1, uv0).rgb*vec3(2.0)*(ambientLightColor.rgb + mergedLightDiffuseColor.rgb), 0.0, 1.0), 0.0, 1.0);
