...
void main() {
  ...
  vec3 colorComponent = clamp(color_GxMaterialColor2*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0.0, 1.0) + color_GxColorRegister5, 0.0, 1.0);

  float alphaComponent = scalar_GxAlphaRegister5;
