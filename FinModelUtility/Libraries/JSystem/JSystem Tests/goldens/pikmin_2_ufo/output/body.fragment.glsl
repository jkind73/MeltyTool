...
uniform vec3 color_GxColorRegister0;
...
void main() {
  ...
  vec3 colorComponent = clamp(clamp(color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0.0, 1.0)*texture(texture1, uv0).rgb*vec3(2.0), 0.0, 1.0) + texture(texture2, uv0).rgb*clamp((color_GxColorRegister0 + texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).rgb*(vec3(1.0) + vec3(-1.0)*texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).rgb) + texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).rgb*texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).rgb)*vec3(2.0), 0.0, 1.0)*color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0.0, 1.0), 0.0, 1.0);
