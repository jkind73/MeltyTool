...
void main() {
  ...
  vec3 colorComponent = clamp((color_GxColorRegister3*(vec3(1.0) + vec3(-1.0)*clamp(texture(texture1, uv1).rgb*vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0.0, 1.0), 0.0, 1.0)) + texture(texture0, uv0).rgb*clamp(texture(texture1, uv1).rgb*vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0.0, 1.0), 0.0, 1.0))*vec3(2.0), 0.0, 1.0);

  float alphaComponent = scalar_GxMaterialAlpha3*texture(texture0, uv0).a;
