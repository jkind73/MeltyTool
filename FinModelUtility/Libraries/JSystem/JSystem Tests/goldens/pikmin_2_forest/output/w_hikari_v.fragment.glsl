...
void main() {
  ...
  vec3 colorComponent = color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0.0, 1.0)*clamp((color_GxColorRegister6*(vec3(1.0) + vec3(-1.0)*texture(texture0, uv0).rgb) + color_GxColorRegister5*texture(texture0, uv0).rgb), 0.0, 1.0);
