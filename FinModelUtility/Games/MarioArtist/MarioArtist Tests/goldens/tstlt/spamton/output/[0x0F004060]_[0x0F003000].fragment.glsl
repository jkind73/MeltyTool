...
void main() {
  ...
  vec3 colorComponent = ((texture_3point(texture1, uv1).rgb + vec3(-1.0)*vec3(0.7843137))*texture_3point(texture0.sampler, clamp(uv0, texture0.clampMin, texture0.clampMax)).rgb + texture_3point(texture0.sampler, clamp(uv0, texture0.clampMin, texture0.clampMax)).rgb)*mergedLightDiffuseColor.rgb;
