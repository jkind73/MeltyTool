...
void main() {
  ...
  vec3 colorComponent = clamp(color_GxColorRegister3 + texture(texture0, uv0).rgb*clamp((color_GxMaterialColor13*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor13), 0.0, 1.0) + texture(texture1, uv0).rgb*vec3(scalar_GxAlphaRegister5) + vec3(-1.0)*texture(texture1, uv0).rgb*vec3(scalar_GxAlphaRegister5)), 0.0, 1.0), 0.0, 1.0);
