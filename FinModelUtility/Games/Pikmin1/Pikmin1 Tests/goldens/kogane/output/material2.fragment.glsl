...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*color_GxMaterialColor2*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor2), 0.0, 1.0) + vec3(-0.5) + color_GxColorRegister6*texture(texture1, sphericalReflectionUv).rgb, 0.0, 1.0);

  float alphaComponent = scalar_GxMaterialAlpha2;
