...
uniform vec3 color_GxMaterialColor15;
uniform vec3 color_GxAmbientColor15;
uniform float scalar_GxMaterialAlpha15;
...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*color_GxMaterialColor15*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor15), 0.0, 1.0), 0.0, 1.0);

  float alphaComponent = texture(texture0, uv0).a*scalar_GxMaterialAlpha15;

  fragColor = vec4(colorComponent, 1);

  if (!(alphaComponent >= 0.5)) {
    discard;
  }
}