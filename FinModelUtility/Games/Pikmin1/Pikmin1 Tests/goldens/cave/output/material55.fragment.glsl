...
uniform vec3 color_GxAmbientColor55;
...
void main() {
  ...
  vec3 colorComponent = vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor55), 0.0, 1.0)*(vec3(1.0) + vec3(-1.0)*texture(texture0.sampler, texture0.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb*vec3(0.5)*texture(texture1.sampler, texture1.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb)*vec3(0.5);
  ...
  if (!(alphaComponent <= 0.1960784 || alphaComponent >= 0.4705882)) {
    discard;
  }
}