...
uniform vec3 color_GxAmbientColor86;
...
void main() {
  ...
  vec3 colorComponent = vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor86), 0.0, 1.0)*(vec3(1.0) + vec3(-1.0)*texture(texture0.sampler, texture0.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb*vec3(2.0)*texture(texture1.sampler, texture1.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb)*vec3(4.0);

  float alphaComponent = texture(texture0.sampler, texture0.transform2d * vec3((uv0).x, (uv0).y, 1)).a*vertexColor0.a*texture(texture1.sampler, texture1.transform2d * vec3((uv0).x, (uv0).y, 1)).a*0.5;

  fragColor = vec4(colorComponent, alphaComponent);

  if (!(alphaComponent <= 0.1372549 || alphaComponent >= 0.3921569)) {
    discard;
  }
}