...
uniform vec3 color_GxAmbientColor6;
uniform float scalar_GxMaterialAlpha6;
uniform float scalar_GxAmbientAlpha6;
...
void main() {
  ...
  vec3 colorComponent = clamp((texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).rgb + vec3(texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).a)*color_GxMaterialColor6*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor6), 0.0, 1.0) + vec3(0.5))*vec3(2.0), 0.0, 1.0);

  float alphaComponent = texture(texture0.sampler, texture0.transform2d * vec3((sphericalReflectionUv).x, (sphericalReflectionUv).y, 1)).a*scalar_GxMaterialAlpha6*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha6);
