...
internal static class StringExtensions {
  ...
  public static string Replace(this string source,
    ...
    int startingPos = 0;
    int nextMatch;
    while ((nextMatch = source.IndexOf(oldValue, startingPos, comparisonType)) >
           -1) {
  ...
  public static string NormalizeSlashes(this string path) {
    ...
    var prefixSeps
        = new string(path.TakeWhile(c => c == Path.DirectorySeparatorChar)
                         .ToArray());
    ...
    if (ImaginaryUnixSupport.IsWindowsPlatform() && prefixSeps.Length >= 2) {
