...
internal static class CommonExceptions {
    ...
    =>
        new DirectoryNotFoundException(
            string.Format(
                CultureInfo.InvariantCulture,
                StringResources.Manager.GetString(
                    "COULD_NOT_FIND_PART_OF_PATH_EXCEPTION"),
                path
            )
        );
  ...
  public static ArgumentException InvalidAccessCombination(
      FileMode mode,
      FileAccess access)
    => new ArgumentException(
        string.Format(
            StringResources.Manager.GetString("INVALID_ACCESS_COMBINATION"),
            mode,
            access),
        nameof(access));
  ...
  public static ArgumentException AppendAccessOnlyInWriteOnlyMode()
    => new ArgumentException(
        string.Format(
            StringResources.Manager.GetString(
                "APPEND_ACCESS_ONLY_IN_WRITE_ONLY_MODE")),
        "access");
  ...
  public static IOException CannotCreateBecauseSameNameAlreadyExists(
      ...
      new IOException(
          string.Format(
              CultureInfo.InvariantCulture,
              StringResources.Manager.GetString(
                  "CANNOT_CREATE_BECAUSE_SAME_NAME_ALREADY_EXISTS"),
              path
          )
      );
  ...
  public static IOException NameCannotBeResolvedByTheSystem(string path) =>
      new IOException(
          string.Format(
              CultureInfo.InvariantCulture,
              StringResources.Manager.GetString(
                  "NAME_CANNOT_BE_RESOLVED_BY_THE_SYSTEM"),
              path
          )
      );

  public static DirectoryNotFoundException PathDoesNotExistOrCouldNotBeFound(
      string path) =>
      new DirectoryNotFoundException(
          string.Format(
              CultureInfo.InvariantCulture,
              StringResources.Manager.GetString(
                  "PATH_DOES_NOT_EXIST_OR_COULD_NOT_BE_FOUND"),
              path
          )
      );
}